...
\centering
\includegraphics[scale=.5]{data/ast-scatter.pdf}
\end{column}
\begin{column}{0.5\textwidth}
\begin{itemize}
\item Total: 3702
\item $\operatorname{ast}(\mathrm{gcn}) = \operatorname{ast}(\mathrm{freq})$: 1899
\item $\operatorname{ast}(\mathrm{gcn}) < \operatorname{ast}(\mathrm{freq})$: 981
\item $\operatorname{ast}(\mathrm{gcn}) > \operatorname{ast}(\mathrm{freq})$: 822
\end{itemize}
\end{column}
\end{columns}
}
